...
require('getmac').getMac(function(err, macAddress) {
    ...
    require('properties').parse(configFile, { path: true }, function (err, config){
        ...
        if(config){
          topic_sub = 'iot-2/cmd/trigger/fmt/json';
...
function sendMessage() {
    ...
    //read the CPU temp from sysfs
        message.d.value = (Math.random() * 30) + 60;
        message.d.sensor_d = 'sensor-' + (Math.floor(Math.random() * 4530) + 6540);
        message.d.timestamp = Date.now();
        console.log(message);
        console.log(topic);
